...
bool Ota::CheckVersion() {
    ...
    ESP_LOGI(TAG, "DIAG_OTA CheckVersion before Open: free_heap=%u min_free=%u psram_free=%u",
             esp_get_free_heap_size(), esp_get_minimum_free_heap_size(), heap_caps_get_free_size(MALLOC_CAP_SPIRAM));
    ...
    if (!http->Open(method, check_version_url_, post_data)) {
        ESP_LOGE(TAG, "Failed to open HTTP connection - free_heap=%u min_free=%u psram_free=%u",
                 esp_get_free_heap_size(), esp_get_minimum_free_heap_size(), heap_caps_get_free_size(MALLOC_CAP_SPIRAM));
        delete http;
        return false;
    }
...
void Ota::Upgrade(const std::string& firmware_url) {
    ...
    ESP_LOGI(TAG, "DIAG_OTA Upgrade before Open: free_heap=%u min_free=%u psram_free=%u",
             esp_get_free_heap_size(), esp_get_minimum_free_heap_size(), heap_caps_get_free_size(MALLOC_CAP_SPIRAM));
    ...
    ESP_LOGI(TAG, "DIAG_OTA Upgrade after Open: free_heap=%u min_free=%u psram_free=%u",
             esp_get_free_heap_size(), esp_get_minimum_free_heap_size(), heap_caps_get_free_size(MALLOC_CAP_SPIRAM));
    char buffer[512];
