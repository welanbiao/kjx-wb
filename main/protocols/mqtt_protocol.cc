...
bool MqttProtocol::SendText(const std::string& text) {
    ...
    if (!mqtt_->Publish(publish_topic_, text)) {
        ...
        // Do not SetError/Alert here: low-memory publish failures would spam UI/audio
        return false;
    }
    return true;
}
...
bool MqttProtocol::OpenAudioChannel() {
    ...
    udp_->OnMessage([this](const std::string& data) {
        ...
        // Drop duplicates / already-played packets
        if (sequence <= remote_sequence_) {
            ESP_LOGW(TAG, "Received audio packet with old sequence: %" PRIu32 ", last: %" PRIu32,
                     sequence, remote_sequence_);
            return;
        }
        // Allow a small forward gap (loss/reorder); reject large jumps
        if (sequence > expected_sequence + 10) {
            ESP_LOGW(TAG, "Received audio packet with out-of-range sequence: %" PRIu32 ", expected: %" PRIu32,
                     sequence, expected_sequence);
            return;
        }
        if (sequence != expected_sequence) {
            ESP_LOGW(TAG, "Audio sequence gap: got %" PRIu32 ", expected %" PRIu32 " (skip ahead)",
                     sequence, expected_sequence);
        }
        ...
        // Always advance past gaps; waiting for a lost packet permanently mutes TTS
        remote_sequence_ = sequence;
        last_incoming_time_ = std::chrono::steady_clock::now();
    });

    if (!udp_->Connect(udp_server_, udp_port_)) {
        ESP_LOGE(TAG, "Failed to connect to UDP server %s:%d", udp_server_.c_str(), udp_port_);
        delete udp_;
        udp_ = nullptr;
        return false;
    }
